        ...
        defer c.Close()
                ...
                //webrtc stuffffffffff
                      ...
                      //  }
                        ...
                        dt := time.Now()
                        log.Print(dt.String())
                        //Wait for ICE gathering to find the best candidate for the LocalDescription(non-trickle ICE)
                        <-gatherComplete
                        ...
                        //write offer back to browser through the websocket connection
                        err = c.WriteMessage( 1, []byte( signal.Encode(*peerConnection.LocalDescription()) ) )
                                if err != nil {
                                  log.Println("write:", err)
                                }


                        //Wait for browser's answer SDP
                       //ReadMessage blocks until message received
                 msgType, message, err2 := c.ReadMessage()
                        if err2 != nil {
                ...
                answer := webrtc.SessionDescription{}
                        ...
                        signal.Decode(SDP, &answer)   //set answer to the decoded SDP
                        log.Print(answer)
